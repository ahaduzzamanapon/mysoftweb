...
class Blog_article extends Backend_Controller {
	...
	public function index(){
        redirect('admin/blog_article/all');
	}

    public function all(){
        $this->data['results'] = $this->db->order_by('sort_order', 'asc')->get('blog_article')->result(); 
        // print_r($this->data['results']); exit;
        //Load page
        $this->data['meta_title'] = 'All Blog Article';
        $this->data['subview'] = 'blog_article/all';
        $this->load->view('backend/_layout_main', $this->data);
    }

    public function details($id){
        $this->data['info'] = $this->Blog_article_model->get_info($id);

        $this->data['meta_title'] = 'Blog Article Details';
        $this->data['subview'] = 'blog_article/details';
        $this->load->view('backend/_layout_main', $this->data);
    }
    ...
    public function edit($id){
        ...
        if ($this->form_validation->run() == true){
            ...
            if($_FILES['userfile']['size'] > 0){
                ...
                $this->Blog_article_model->delete_img($id);
            ...
            if($this->Common_model->edit('blog_article', $id, 'id', $form_data)){
                ...
                foreach ($query as $key => $value) {
                    ...
                    $this->db->update('blog_article', array('sort_order' => $sort+$key+1));
                }
                $this->db->where('id', $id);
                $this->db->update('blog_article', array('sort_order' => $this->input->post('sort_order')));



                $this->session->set_flashdata('success', 'Blog Information update successfully.');
                redirect('admin/blog_article/all');
            }
        }

        $this->data['meta_title'] = 'Edit Blog';
        $this->data['subview'] = 'blog_article/edit';
        $this->load->view('backend/_layout_main', $this->data);
    }
	...
	public function add(){
        ...
        if ($this->form_validation->run() == true){
            ...
            if($this->Common_model->save('blog_article', $form_data)){                
                $this->session->set_flashdata('success', 'New blog save successfully.');
               redirect("admin/blog_article/all");
            }
        }

		$this->data['meta_title'] = 'Add Blog';
		$this->data['subview'] = 'blog_article/add';
    	$this->load->view('backend/_layout_main', $this->data);
	}
    ...
    function delete($id) {
        $this->data['info'] = $this->Blog_article_model->delete($id);
