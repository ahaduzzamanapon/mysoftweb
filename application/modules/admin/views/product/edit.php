            ...
            <div class="row">
              <div class="col-md-7">
                  ...
                  <div class="form-group">
                    ...
                    <textarea id='overviewEdit' name="overview" rows="10" cols="80"><?=set_value('overview',$info->overview)?></textarea>
                  </div>

                  <div class="form-group">
                    <label>Modules</label>
                    <div><?php echo form_error('modules'); ?></div>
                    <textarea id="modulesEdit" name="modules" rows="10" cols="80"><?=set_value('modules',$info->modules)?></textarea>
                  </div>

                  <div class="form-group">
                    <label>Features</label>
                    <div><?php echo form_error('features'); ?></div>
                    <textarea id="featuresEdit" name="features" rows="10" cols="80"><?=set_value('features',$info->features)?></textarea>
                  </div>

                  <div class="form-group">
                    <label>Technology</label>
                    <div><?php echo form_error('technology'); ?></div>
                    <textarea id="technologyEdit" name="technology" rows="10" cols="80"><?=set_value('technology',$info->technology)?></textarea>
                  </div>
              </div>
              ...
              <div class="col-md-5">
                <div class="form-group">
                    ...
                    // print_r($info->user_client); set_value('user_client', $info->user_client)
                    // $selected = array();
                    //     foreach($info->user_client as $row){
                    //         $selected[] = $row['id'];
                    //     }
                    $selected = explode(',', $info->user_client);

                    $att = array('class' => "form-control");
                    echo form_multiselect('user_client[]', $clients, $selected, $att); ?>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" rows="3" name="demo_url" placeholder="Demo Url"><?=set_value('demo_url')?><?= $info->demo_url ?></textarea>
                </div>
...
ClassicEditor.create(document.querySelector('#overviewEdit'))
    .then(editor => {
...
ClassicEditor.create(document.querySelector('#modulesEdit'))
    .then(editor => {
        window.editor = editor;
    })
    .catch(error => {
...
ClassicEditor.create(document.querySelector('#featuresEdit'))
    .then(editor => {
        window.editor = editor;
    })
    .catch(error => {
...
ClassicEditor.create(document.querySelector('#technologyEdit'))
    .then(editor => {
        window.editor = editor;
    })
    .catch(error => {
