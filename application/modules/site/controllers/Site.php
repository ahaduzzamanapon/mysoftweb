...
class Site extends Frontend_Controller
{
    ...
    public function hire_talent_get_touch()
    {
        ...
        $this->form_validation->set_rules('budget', 'Budget Range', 'required|trim');
        $this->form_validation->set_rules('details', 'Project Details', 'required|trim');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($this->Common_model->save('get_touch_hire_talent', $form_data)) {

                // ✅ send mail to admin with inserted data
                $a = $this->send_hire_talent_mail($form_data);
                // dd($a);
                ...
                $this->session->set_flashdata('success', 'Thank you! Your inquiry has been sent successfully.');
    ...
    private function send_hire_talent_mail($data)
    {
        $this->load->library('email');

        $message = "
            <h2>New Hire Talent Inquiry</h2>
        ...
        $config = array(
            ...
            'smtp_user' => 'info.hrseba@gmail.com',
            'smtp_pass' => 'jkldsgqoniylnqph',
        ...
        $this->email->from('info.hrseba@gmail.com', 'Hire Talent Form');

        // ✅ Only admin receivers
        $this->email->to([
            'md@mysoftheaven.com',
            'pollob8@gmail.com',
            'info@mysoftheaven.com',
            'afif.mysoftheaven@gmail.com',
            'masud.r.mysoftheaven@gmail.com'
        ]);

        // ❌ removed reply_to (optional — remove if you don’t want it)
        // $this->email->reply_to($data['email'], $data['name']);

        $this->email->subject('New Hire Talent Request');
        $this->email->message($message);

        if (!$this->email->send()) {
            log_message('error', 'Hire Talent Mail Failed: ' . $this->email->print_debugger());
            return false;
        }
